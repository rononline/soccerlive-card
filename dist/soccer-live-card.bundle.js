...
`;async function we(e){if(!e||"N/A"===e)return q``;try{const t=await async function(e){if(!e||"N/A"===e)return null;if(ve.has(e))return ve.get(e);if(_e[e])return ve.set(e,_e[e]),_e[e];try{const t=await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(e+" stadium")}&format=json&limit=1`,{headers:{"Accept-Language":"en"}}),a=await t.json();if(a&&a.length>0){const t={lat:parseFloat(a[0].lat),lon:parseFloat(a[0].lon)};return ve.set(e,t),t}}catch(e){console.warn("Geocoding failed:",e)}return null}(e);if(!t)return q``;const a=await async function(e,t){const a=`${e},${t}`;if(be.has(a)){const e=be.get(a);if(Date.now()-e.timestamp<36e5)return e.data}try{const s=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${e}&longitude=${t}&current=temperature_2m,weather_code,wind_speed_10m&timezone=auto`),n=await s.json();if(n.current){const e={temp:Math.round(n.current.temperature_2m),code:n.current.weather_code,wind:Math.round(n.current.wind_speed_10m),icon:(i=n.current.weather_code,i&&0!==i&&1!==i?2===i?"⛅":3===i?"☁️":45===i||48===i?"🌫️":51===i||53===i||55===i||61===i||63===i||65===i?"🌧️":71===i||73===i||75===i||77===i?"🌨️":80===i||81===i||82===i?"🌧️":85===i||86===i?"🌨️":95===i||96===i||99===i?"⛈️":"🌤️":"☀️"),description:xe(n.current.weather_code),timestamp:Date.now()};return be.set(a,{data:e,timestamp:Date.now()}),e}}catch(e){console.warn("Weather fetch failed:",e)}var i;return null}(t.lat,t.lon);return a?q`
      <div class="weather-badge" title="${e}: ${a.description}">
        <span class="weather-icon">${a.icon}</span>
        <span class="weather-temp">${a.temp}°</span>
        <span class="weather-wind" title="Wind speed">${a.wind} km/h</span>
      </div>
