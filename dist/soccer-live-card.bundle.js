...
`;customElements.define("soccer-live-standings-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array},groups:{type:Array}}}constructor(){super(),this.entities=[],this.groups=[]}static get styles(){return[_e,s`
    ...
    `}updated(e){e.has("hass")&&this.hass&&!this._eventSubscriptions?.length&&this._subscribeToEvents(),e.has("showPopup")&&(this.showPopup?this._escHandler||(this._escHandler=e=>{"Escape"===e.key&&(this.showPopup=!1)},document.addEventListener("keydown",this._escHandler)):this._escHandler&&(document.removeEventListener("keydown",this._escHandler),this._escHandler=null))}_renderPopup(){const e=this.activeMatch,t=e=>null==e||""===e||"N/A"===e?"-":e,a=e.clock&&"N/A"!==e.clock?e.clock:e.status&&"N/A"!==e.status?e.status:"",{goals:i,yellowCards:n,redCards:s}=this.separateEvents(e.match_details||[]),o=(e,t,a)=>t.length?R`
    ...
    `]}}),customElements.define("soccer-live-matches-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return[_e,s`
      .card-config {
        display: flex;
        flex-direction: column;
        gap: 16px;
...
`;async function ze(e){let t=arguments.length>1&&void 0!==arguments[1]?arguments[1]:null,a=arguments.length>2&&void 0!==arguments[2]?arguments[2]:null,i=arguments.length>3&&void 0!==arguments[3]?arguments[3]:null,n=arguments.length>4&&void 0!==arguments[4]?arguments[4]:null;if(!e||"N/A"===e)return R``;try{let s=null;if(s=null!==i&&null!==n?{lat:i,lon:n}:await function(e){if(!e||"N/A"===e)return null;if(be.has(e))return be.get(e);const t=we[e]||null;return t&&($e(be),be.set(e,t)),t}(e),!s)return R``;const o=await async function(e,t){const a=`${e},${t}`;if(xe.has(a)){const e=xe.get(a);if(Date.now()-e.timestamp<36e5)return e.data}try{const s=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${e}&longitude=${t}&current=temperature_2m,weather_code,wind_speed_10m&timezone=auto`),o=await s.json();if(o.current){const e={temp:Math.round(o.current.temperature_2m),code:o.current.weather_code,wind:(n=o.current.wind_speed_10m,n<1?0:n<6?1:n<12?2:n<20?3:n<29?4:n<39?5:n<50?6:n<62?7:n<75?8:n<89?9:n<103?10:n<118?11:12),wind_unit:"BFT",icon:(i=o.current.weather_code,i&&0!==i&&1!==i?2===i?"⛅":3===i?"☁️":45===i||48===i?"🌫️":i>=51&&i<=55?"🌦️":i>=61&&i<=65?"🌧️":i>=71&&i<=77?"🌨️":i>=80&&i<=82?"🌧️":85===i||86===i?"🌨️":95===i||96===i||99===i?"⛈️":"🌤️":"☀️"),description:ke(o.current.weather_code),timestamp:Date.now()};return $e(xe),xe.set(a,{data:e,timestamp:Date.now()}),e}}catch(e){console.warn("Weather fetch failed:",e)}var i,n;return null}(s.lat,s.lon);if(!o)return R``;const r=t?pe("weather.wind",de(t,a)):"Wind speed (Beaufort)";return R`
    ...
    `}catch(e){return console.warn("Weather badge error:",e),R``}}const Ee=s`
  .spinner-container {
    display: flex;
    align-items: center;
    justify-content: center;
...
`},Ne="soccer_live_cache_",Le=new Map;class Pe{static set(e,t){try{const a=JSON.stringify(t);if(Le.get(e)===a)return;Le.size>=50&&Le.delete(Le.keys().next().value),Le.set(e,a),localStorage.setItem(Ne+e,JSON.stringify({timestamp:Date.now(),data:t}))}catch(e){console.warn("Failed to cache:",e)}}static get(e){try{const t=localStorage.getItem(Ne+e);if(!t)return null;const{timestamp:a,data:i}=JSON.parse(t),n=Date.now()-a;return n<864e5?{data:i,age:n,isCached:!0}:(this.clear(e),null)}catch(e){return console.warn("Failed to read cache:",e),null}}static clear(e){try{localStorage.removeItem(Ne+e)}catch(e){console.warn("Failed to clear cache:",e)}}static clearAll(){try{Object.keys(localStorage).forEach((e=>{e.startsWith(Ne)&&localStorage.removeItem(e)}))}catch(e){console.warn("Failed to clear all cache:",e)}}static getAge(e){const t=localStorage.getItem(Ne+e);if(!t)return null;try{const{timestamp:e}=JSON.parse(t),a=Date.now()-e,i=Math.floor(a/6e4);if(i<60)return`${i}m ago`;const n=Math.floor(i/60);return n<24?`${n}h ago`:`${Math.floor(n/24)}d ago`}catch(e){return null}}}customElements.define("soccer-live-team",class extends oe{static get properties(){return{hass:{},_config:{},_isLoading:{type:Boolean},showPopup:{type:Boolean},activeMatch:{type:Object},_eventSubscriptions:{type:Array},_toastMessage:{type:String},_toastVisible:{type:Boolean},_toastVariant:{type:String},_weatherBadge:{type:Object},_cachedData:{type:Object}}}setConfig(e){if(!e.entity)throw new Error("Entity required");this._config=e,me(this,e);const t=["big","huge"].includes(e.score_size)?e.score_size:"normal";this.setAttribute("data-score",t),this._isLoading=!0,this._loadingStarted=Date.now(),this._lastWeatherVenue=null,this.showPopup=!1,this.activeMatch=null,this.showEventToasts=!0===e.show_event_toasts,this.myTeam=(e.my_team||"").toLowerCase(),this.showPreviousMatches=!0===e.show_previous_matches,this.showFormTrend=!0===e.show_form_trend,this._toastMessage="",this._toastVisible=!1,this._toastVariant="goal",this._toastTimer=null,this._animationTimers||(this._animationTimers=[])}_t(e,t){return pe(e,de(this.hass,this._config),t)}_translatePhase(e){return e?{"regular-season":this._t("phase.regular_season"),"regular season":this._t("phase.regular_season"),"group-stage":this._t("phase.group_stage"),"group stage":this._t("phase.group_stage"),playoffs:this._t("phase.playoffs")}[String(e).toLowerCase()]||e:""}_shouldShowPhase(e){return!!e&&"regular-season"!==String(e).toLowerCase()}connectedCallback(){super.connectedCallback(),this._subscribeToEvents(),this._countdownInterval=setInterval((()=>this.requestUpdate()),3e4),this._loadingTimer=setTimeout((()=>this.requestUpdate()),1e4)}disconnectedCallback(){super.disconnectedCallback(),clearInterval(this._countdownInterval),this._countdownInterval=null,clearTimeout(this._loadingTimer),this._eventSubscriptions&&Array.isArray(this._eventSubscriptions)&&(this._eventSubscriptions.forEach((e=>{"function"==typeof e&&e()})),this._eventSubscriptions=[]),this._escHandler&&(document.removeEventListener("keydown",this._escHandler),this._escHandler=null),clearTimeout(this._toastTimer),this._toastTimer=null,this._animationTimers&&(this._animationTimers.forEach((e=>clearTimeout(e))),this._animationTimers=[])}_subscribeToEvents(){this.hass&&this.hass.connection&&(this._eventSubscriptions&&this._eventSubscriptions.length>0||(this._eventSubscriptions=[],["soccer_live_goal","soccer_live_yellow_card","soccer_live_red_card"].forEach((e=>{this.hass.connection.subscribeEvents(this._handleSoccerLiveEvent.bind(this),e).then((e=>{"function"==typeof e&&this._eventSubscriptions.push(e)}))}))))}_eventBelongsToThisCard(e){if(!this.hass||!this._config)return!1;const t=this.hass.states[this._config.entity];if(!t)return!1;const a=t.attributes.matches||[];if(0===a.length)return!1;const i=a[0];return i.home_team===e.home_team&&i.away_team===e.away_team}_handleSoccerLiveEvent(e){const t=e.event_type,a=e.data;if(this._eventBelongsToThisCard(a)&&this.showEventToasts)if("soccer_live_goal"===t){const e=a.team===a.home_team?"home":"away";requestAnimationFrame((()=>this._triggerGoalCelebration(e,a)))}else this._showEventToast(t,a)}_showEventToast(e,t){let a="",i="goal";"soccer_live_goal"===e?(a=`<strong>${this._t("event.goal").toUpperCase()}!</strong> ${t.player} · ${t.home_team} ${t.home_score} - ${t.away_score} ${t.away_team}`,i="goal"):"soccer_live_yellow_card"===e?(a=`🟨 <strong>${this._t("event.yellow_card")}</strong> · ${t.player}${t.minute?` (${t.minute}')`:""}`,i="yellow"):"soccer_live_red_card"===e&&(a=`🟥 <strong>${this._t("event.red_card")}</strong> · ${t.player}${t.minute?` (${t.minute}')`:""}`,i="red"),a&&(this._toastMessage=a,this._toastVariant=i,this._toastVisible=!0,this._toastTimer&&clearTimeout(this._toastTimer),this._toastTimer=setTimeout((()=>{this._toastVisible=!1,this.requestUpdate()}),4e3),this.requestUpdate())}_triggerGoalCelebration(e,t){const a=this.shadowRoot&&this.shadowRoot.querySelector("ha-card");if(!a)return;a.querySelectorAll(".confetti, .goal-banner, .goal-flash-overlay").forEach((e=>e.remove())),a.classList.remove("goal-flash"),a.offsetWidth,a.classList.add("goal-flash"),this._animationTimers.push(setTimeout((()=>a.classList.remove("goal-flash")),1700));const i=document.createElement("div");i.className="goal-flash-overlay",a.appendChild(i),this._animationTimers.push(setTimeout((()=>i.remove()),1e3));const n=document.createElement("div");n.className="goal-banner";const s=document.createElement("div");s.className="goal-banner-text",s.textContent="GOAL!",n.appendChild(s),a.appendChild(n),this._animationTimers.push(setTimeout((()=>n.remove()),1700));const o=a.querySelector(".score-numbers");o&&(o.classList.remove("goal-scored"),o.offsetWidth,o.classList.add("goal-scored"),this._animationTimers.push(setTimeout((()=>o.classList.remove("goal-scored")),1300)));const r=a.querySelectorAll(".team-side .team-logo-big"),l="away"===e?r[1]:r[0];l&&(l.classList.remove("scorer-bounce"),l.offsetWidth,l.classList.add("scorer-bounce"),this._animationTimers.push(setTimeout((()=>l.classList.remove("scorer-bounce")),1300))),navigator.vibrate&&navigator.vibrate([180,80,180,80,280]),this._animationTimers.push(setTimeout((()=>this._showEventToast("soccer_live_goal",t)),600));const c=["#ec4899","#6366f1","#06b6d4","#fbbf24","#10b981","#ef4444"],d=["⚽","🎉","✨","🔥","⭐"];for(let e=0;e<36;e++){const e=document.createElement("div");e.className="confetti",Math.random()>.55?(e.textContent=d[Math.floor(Math.random()*d.length)],e.style.fontSize=14+10*Math.random()+"px",e.style.background="transparent"):(e.style.background=c[Math.floor(Math.random()*c.length)],e.style.borderRadius=Math.random()>.5?"50%":"2px");const t=380*(Math.random()-.5)+"px",i=240*Math.random()+100+"px";e.style.setProperty("--dx",t),e.style.setProperty("--dy",i),e.style.animationDelay=.3*Math.random()+"s",a.appendChild(e),this._animationTimers.push(setTimeout((()=>e.remove()),2e3))}}_parseMatchDate(e){if(!e||"N/A"===e)return null;try{const[t,a]=e.split(" "),[i,n,s]=t.split(/[-\/]/).map(Number),[o,r]=(a||"00:00").split(":").map(Number);return new Date(s,n-1,i,o,r)}catch(e){return null}}_liveCountdown(e){if(!e||"pre"!==e.state)return null;const t=this._parseMatchDate(e.date);if(!t)return null;const a=t-new Date;if(a<=0||a>1728e5)return null;const i=Math.floor(a/6e4);if(i<1)return this._t("time.now");if(i<60)return this._t("time.in_n_min",{n:i});const n=Math.floor(i/60);return this._t("time.in_n_h",{n})}getCardSize(){return 4}static getConfigElement(){return document.createElement("soccer-live-team-editor")}static getStubConfig(){return{entity:"sensor.soccer_live_next_",show_event_toasts:!1}}showDetails(e){this.activeMatch=e,this.showPopup=!0}closePopup(){this.showPopup=!1}separateEvents(e){const t=[],a=[],i=[];return e.forEach((e=>{const n=String(e||"");n.includes("Goal")||n.includes("Penalty - Scored")?t.push(this.formatMatchEvent(n)):n.includes("Yellow Card")?a.push(this.formatMatchEvent(n)):n.includes("Red Card")&&i.push(this.formatMatchEvent(n))})),{goals:t,yellowCards:a,redCards:i}}formatMatchEvent(e){const t=e=>this._t(e);let a=String(e||"").trim();a=a.replace(/^Goal\s*-\s*/i,"").replace(/^Yellow Card\s*-\s*/i,"").replace(/^Red Card\s*-\s*/i,"").replace(/^Penalty - Scored\s*-\s*/i,`${t("event.penalty")} - `).replace(/^Header\s*-\s*/i,`${t("event.header")} - `).replace(/^Shot\s*-\s*/i,`${t("event.shot")} - `).replace(/^Free-kick\s*-\s*/i,`${t("event.free_kick")} - `).replace(/^Penalty\s*-\s*/i,`${t("event.penalty")} - `),a=a.replace(/^([^:]+):\s*/,"$1 ");const i=[t("event.header"),t("event.shot"),t("event.penalty"),t("event.free_kick")].map((e=>e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));return a=a.replace(new RegExp(`^(${i.join("|")})\\s*-\\s*(.+)$`,"i"),((e,t,a)=>`${a} (${t.toLowerCase()})`)),a=a.replace(/\bN\/A\b/g,t("generic.unknown")),a}_renderStatusBadge(e){const t=e.state;return"in"===t?R`<span class="status-badge live"><span class="dot"></span>${this._t("status.live")}</span>`:"post"===t?R`<span class="status-badge finished">${this._t("status.finished")}</span>`:R`<span class="status-badge scheduled">${e.date||this._t("status.scheduled")}</span>`}_renderClock(e){const t=e.state;if("in"===t){const t=e.status_detail&&"N/A"!==e.status_detail?e.status_detail:"",a=(e.clock&&"N/A"!==e.clock?e.clock:"")||t||e.status||"";return R`<div class="clock"><span class="dot"></span>${a}</div>`}if("post"===t)return R`<div class="clock finished">${this._t("status.full_time")}</div>`;const a=this._liveCountdown(e);return R`<div class="clock upcoming">${a||e.date||""}</div>`}_renderRecord(e){if(!e||"N/A"===e)return"";const t=String(e).split("-");return 3===t.length?t.every((e=>0===parseInt(e)))?"":R`<div class="record">
    ...
    `}_renderPopupLineup(e){const t=e.lineup_home||[],a=e.lineup_away||[];if(!t.length&&!a.length)return"";const i=(e,t,a)=>{const i=(e||[]).filter((e=>e.starter));return i.length?R`
      ...
      </div>`}static get styles(){return[he,Ee,Ce,s`
    ...
    `]}}),customElements.define("soccer-live-team-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return[_e,s`
      .card-config {
        display: flex;
        flex-direction: column;
        gap: 16px;
    ...
    `]}}),customElements.define("soccer-live-news-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return s`
    ...
    `]}}),customElements.define("soccer-live-lineup-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return[_e,s`
      .card-config { display: flex; flex-direction: column; gap: 16px; }
      .option { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
      label { font-size: 14px; color: var(--primary-text-color); }
      .field-label { display: block; font-size: 12px; color: var(--secondary-text-color); margin-bottom: 4px; font-weight: 600; }
    ...
    `]}}),customElements.define("soccer-live-timeline-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return s`
      .card-config { display: flex; flex-direction: column; gap: 16px; }
      .option { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
      label { font-size: 14px; color: var(--primary-text-color); }
      .field-label { display: block; font-size: 12px; color: var(--secondary-text-color); margin-bottom: 4px; font-weight: 600; }
    ...
    `]}}),customElements.define("soccer-live-bracket-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return s`
      .card-config { display: flex; flex-direction: column; gap: 16px; }
      .option { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
      label { font-size: 14px; color: var(--primary-text-color); }
      .field-label { display: block; font-size: 12px; color: var(--secondary-text-color); margin-bottom: 4px; font-weight: 600; }
    ...
    `}}),customElements.define("soccer-live-scorers",class extends oe{static get properties(){return{hass:{},_config:{},_isLoading:{type:Boolean}}}setConfig(e){if(!e.entity)throw new Error("Entity required");this._config=e,me(this,e),this._isLoading=!0}connectedCallback(){super.connectedCallback(),this._loadingTimer=setTimeout((()=>this.requestUpdate()),1e4)}disconnectedCallback(){super.disconnectedCallback(),clearTimeout(this._loadingTimer)}updated(e){if(e.has("hass")&&this.hass&&this._config){const e=this.hass.states[this._config.entity];e&&"unavailable"!==e.state&&(this._isLoading=!1,Pe.set(this._config.entity,e.attributes))}}_t(e){return pe(e,de(this.hass,this._config))}render(){if(!this.hass||!this._config)return Se("Loading...");const e=this._config.entity,t=this.hass.states[e];let a=null;if(t)if("unavailable"===t.state){const t=Pe.get(e);if(!t?.data?.scorers)return Ae("📡","Sensor unavailable","Integration may not be running","Restart Home Assistant");a=t.data}else{if("Not available"===t.state)return Te("📊","Not available","Top scorer data not available for this competition via ESPN","");a=t.attributes}else{const t=Pe.get(e);if(!t?.data?.scorers)return Ae("⚠️","Entity not found",e,"Check entity configuration");a=t.data}return this._isLoading&&!a?Se("Loading scorers..."):this._renderCard(a||{})}_renderCard(e){const t=e.scorers||[],a=this._config.max_items??10,i=!0===this._config.hide_header,n=t.slice(0,a);return n.length?R`
      <ha-card>
        ${i?"":R`
          <div class="slc-header">
            ${e.league_logo?R`<img class="slc-league-logo" src="${e.league_logo}" alt="">`:""}
            <span class="slc-league-name">${e.league_name||"Top Scorers"}</span>
          </div>
        `}
        <div class="slc-list">
          ${n.map(((e,t)=>R`
            <div class="slc-row ${t%2==0?"even":""}">
              ...
              ${e.headshot?R`<img class="slc-headshot" src="${e.headshot}" alt="" @error="${e=>e.target.style.display="none"}">`:R`<div class="slc-headshot placeholder"></div>`}
              <div class="slc-info">
                <span class="slc-name">${e.player||e.short_name||"—"}</span>
                <span class="slc-team">${e.team_name||""}</span>
              </div>
              ${e.team_logo?R`<img class="slc-team-logo" src="${e.team_logo}" alt="" @error="${e=>e.target.style.display="none"}">`:""}
              <span class="slc-goals">${e.goals}</span>
            </div>
          `))}
        </div>
      </ha-card>
    `:Te("🥇","No data","No top scorer data available","")}static getConfigElement(){return document.createElement("soccer-live-scorers-editor")}static getStubConfig(){return{entity:"",max_items:10}}static get styles(){return[he,s`
      ha-card {
    ...
    `]}render(){if(!this.hass||!this._config)return Se("Loading...");const e=this.hass.states[this._config.entity];if(!e)return Pe.get(this._config.entity)?Ae("⏱","Offline - showing cached data","Last update: "+(new Date).toLocaleTimeString(),"Waiting for integration to come online"):Ae("⚠️","Entity not found",`Unable to find: ${this._config.entity}`,"Check the entity configuration");if("unavailable"===e.state){const e=Pe.get(this._config.entity);if(!e)return Ae("📡","Sensor unavailable","The integration may not be running","Restart Home Assistant or check the integration");this._cachedData=e.data}if(this._isLoading)return Date.now()-this._loadingStarted>1e4?Ae("⏱",this._t("ui.loading_timeout"),`${this._t("ui.entity_not_responding")}: ${this._config.entity}`,this._t("ui.check_integration")):Se(this._t("ui.loading"));const t=e&&"unavailable"!==e.state?e.attributes:this._cachedData,a=this._getNextMatch({attributes:t});if(!a)return Te("📅",this._t("ui.off_season"),this._t("ui.off_season_hint"));const i="in"===a.state,n="post"===a.state,s=i||n?null:this._countdown(a.date),o=a.competition_name||t?.league_name||"",r=a.competition_logo||t?.league_logo||"",l=a.venue&&"N/A"!==a.venue?a.venue:"",c=a.venue_city&&"N/A"!==a.venue_city?a.venue_city:"",d=Array.isArray(a.broadcasts)&&a.broadcasts.length?a.broadcasts:a.broadcast&&"N/A"!==a.broadcast?[a.broadcast]:[],p=a.neutral_site||!1,h=this._t("cd.days")||"days",g=this._t("cd.hrs")||"hrs",u=this._t("cd.min")||"min",m=this._t("cd.sec")||"sec";return R`
      <ha-card>
    ...
    `}});const Ve=["dark","light","red-white","classic","neon","gold"],He=["auto","en","nl","de","pt","fr","es","it"];customElements.define("soccer-live-countdown-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return[_e,s`
      .card-config { display: flex; flex-direction: column; gap: 16px; }
      .option { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
      label { font-size: 14px; color: var(--primary-text-color); }
      .field-label { display: block; font-size: 12px; color: var(--secondary-text-color); margin-bottom: 4px; font-weight: 600; }
    ...
    `]}updated(e){if(e.has("hass")){const e=this.hass?.states[this._config?.entity];e&&"unavailable"!==e.state&&(this._isLoading=!1,Pe.set(this._config.entity,e.attributes))}}render(){if(!this.hass||!this._config)return Se("Loading...");const e=this.hass.states[this._config.entity];if(!e)return Pe.get(this._config.entity)?Ae("⏱","Offline - showing cached data","Last update: "+(new Date).toLocaleTimeString(),"Waiting for integration"):Ae("⚠️","Entity not found",`Unable to find: ${this._config.entity}`,"Check the entity configuration");if("unavailable"===e.state){const e=Pe.get(this._config.entity);if(!e||!e.data.matches)return Ae("📡","Sensor unavailable","The integration may not be running","Restart Home Assistant or check the integration");this._cachedData=e.data}if(this._isLoading)return Date.now()-this._loadingStarted>1e4?Ae("⏱",this._t("ui.loading_timeout"),`${this._t("ui.entity_not_responding")}: ${this._config.entity}`,this._t("ui.check_integration")):Se(this._t("ui.loading"));const t=e&&"unavailable"!==e.state?e.attributes:this._cachedData,a=t?.matches||[],i=this._getMatch({attributes:t});if(!i)return a.length?Te("⚽",this._t("ui.no_live_match"),this._t("ui.no_live_match_hint")):Te("📅",this._t("ui.off_season"),this._t("ui.off_season_hint"));const n="in"===i.state||"live"===i.status,s="post"===i.state,o=("pre"===i.state||i.status,i.key_events||i.events||i.match_events||[]),r=i.competition_name||t?.league_name||"",l=i.competition_logo||t?.league_logo||"",c=(Array.isArray(i.broadcasts)&&i.broadcasts.length?i.broadcasts:i.broadcast&&"N/A"!==i.broadcast&&i.broadcast,i.neutral_site,i.venue&&"N/A"!==i.venue?i.venue:""),d=i.venue_city&&"N/A"!==i.venue_city?i.venue_city:"";let p=i.stats||[];if(!p.length&&i.home_statistics&&"object"==typeof i.home_statistics){const e=i.home_statistics,t=i.away_statistics||{};p=Object.entries(e).filter((e=>{let[t]=e;return"Unknown"!==t})).map((e=>{let[a,i]=e;return{label:a,home:i,away:t[a]??"N/A"}}))}return R`
      <ha-card>
        <div class="hero">
    ...
    `}});const qe=["dark","light","red-white","classic","neon","gold"],Ue=["auto","en","nl","de","pt","fr","es","it"];customElements.define("soccer-live-team-competitions-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return[_e,s`
      .hint { font-size: 11px; color: var(--secondary-text-color); }
    ...
    `}});const Ge=["dark","light","red-white","classic","neon","gold"],We=["auto","en","nl","de","pt","fr","es","it"];customElements.define("soccer-live-live-commentary-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return[_e,s`
      .hint { font-size: 11px; color: var(--secondary-text-color); }
    ...
    `}});const Ke={team:"soccer-live-team",standings:"soccer-live-standings",matches:"soccer-live-matches",countdown:"soccer-live-countdown","live-match":"soccer-live-live-match",news:"soccer-live-news",lineup:"soccer-live-lineup",timeline:"soccer-live-timeline",bracket:"soccer-live-bracket","mini-standings":"soccer-live-mini-standings",scorers:"soccer-live-scorers","multi-team":"soccer-live-multi-team","team-competitions":"soccer-live-team-competitions","live-commentary":"soccer-live-live-commentary"},Je=[{value:"team",label:"Team",description:"Live score, form, lineup, weather for one team"},{value:"standings",label:"Standings",description:"League table with coloured zones"},{value:"matches",label:"Matches",description:"Day-grouped match list with live highlighting"},{value:"countdown",label:"Countdown",description:"Countdown timer to next match with live score"},{value:"live-match",label:"Live Match",description:"Current match with events, possession and shots"},{value:"news",label:"News",description:"Article feed with images and timestamps"},{value:"lineup",label:"Lineup",description:"Starting eleven for both teams"},{value:"timeline",label:"Timeline",description:"Minute-by-minute match events"},{value:"bracket",label:"Bracket",description:"Knockout bracket (list or tournament tree)"},{value:"mini-standings",label:"Mini Standings",description:"Compact standings with configurable rows"},{value:"scorers",label:"Top Scorers",description:"Top scorers list with photo and goal tally"},{value:"multi-team",label:"Multi Team",description:"Multiple teams in one compact card"},{value:"team-competitions",label:"Team Competitions",description:"All competitions for a team with tab selector"},{value:"live-commentary",label:"Live Commentary",description:"Real-time play-by-play commentary"}],Ze={team:"soccer-live-team-editor",standings:"soccer-live-standings-editor",matches:"soccer-live-matches-editor",countdown:"soccer-live-countdown-editor","live-match":"soccer-live-live-match-editor",news:"soccer-live-news-editor",lineup:"soccer-live-lineup-editor",timeline:"soccer-live-timeline-editor",bracket:"soccer-live-bracket-editor","mini-standings":"soccer-live-mini-standings-editor",scorers:"soccer-live-scorers-editor","multi-team":"soccer-live-multi-team-editor","team-competitions":"soccer-live-team-competitions-editor","live-commentary":"soccer-live-live-commentary-editor"},Qe=["entity","skin","language","show_event_toasts"];class Ye extends HTMLElement{constructor(){super(),this._hass=null,this._config={},this._child=null,this._childType=null}set hass(e){this._hass=e,this._child&&(this._child.hass=e)}setConfig(e){this._config=e||{};const t=this._config.card_type,a=t?Ke[i=t]||(i?.startsWith("soccer-live-")?i:null):null;var i;if(!a)return this._destroyChild(),this.innerHTML="",void this.appendChild(this._placeholder());this._childType!==a&&(this._destroyChild(),this._child=document.createElement(a),this._childType=a,this.innerHTML="",this.appendChild(this._child));try{this._child.setConfig(this._config)}catch(e){}this._hass&&(this._child.hass=this._hass)}_destroyChild(){this._child&&(this._child=null,this._childType=null)}_placeholder(){const e=document.createElement("ha-card");return e.style.cssText="padding:24px;text-align:center;color:#94a3b8;font-size:13px;",e.textContent="Soccer Live — open the editor to choose a card type.",e}getCardSize(){return this._child?.getCardSize?.()??1}getGridOptions(){return this._child?.getGridOptions?.()}static getConfigElement(){return document.createElement("soccer-live-card-editor")}static getStubConfig(){return{}}}customElements.define("soccer-live-card",Ye),customElements.define("soccer-live-card-editor",class extends oe{static get properties(){return{hass:{type:Object},_config:{type:Object}}}constructor(){super(),this._config={},this._subEditor=null,this._subEditorType=null,this._configByType={}}setConfig(e){this._config={...e||{}},this.requestUpdate()}updated(e){e.has("hass")&&this._subEditor&&(this._subEditor.hass=this.hass),e.has("_config")&&this._syncSubEditor()}_syncSubEditor(){const e=this.shadowRoot?.getElementById("sub-editor");if(!e)return;const t=this._config.card_type,a=Ze[t]?t:Object.entries(Ke).find((e=>{let[,a]=e;return a===t}))?.[0]||t,i=Ze[a];if(!a||!i)return e.innerHTML="",this._subEditor=null,void(this._subEditorType=null);if(this._subEditorType!==a){e.innerHTML="",this._subEditor=document.createElement(i),this._subEditorType=a,this._subEditor.hass=this.hass,this._subEditor.addEventListener("config-changed",(e=>{this._dispatch({...e.detail.config,card_type:a})})),e.appendChild(this._subEditor);try{this._subEditor.setConfig(this._config)}catch(e){}}else try{this._subEditor.setConfig(this._config)}catch(e){}}_typeChanged(e){const t=e.target.value;if(!t)return;const a=this._config.card_type;a&&(this._configByType[a]={...this._config});const i=this._configByType[t];if(i)this._dispatch({...i,card_type:t});else{const e={};for(const t of Qe)void 0!==this._config[t]&&(e[t]=this._config[t]);this._dispatch({...e,card_type:t})}}_dispatch(e){const t={...e,type:this._config?.type||"custom:soccer-live-card"};this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}render(){const e=this._config?.card_type||"",t=Je.find((t=>t.value===e))?e:Object.entries(Ke).find((t=>{let[,a]=t;return a===e}))?.[0]||e,a=Je.find((e=>e.value===t));return R`
      <div class="picker-wrap">
        <label class="picker-label">Card type</label>
        <select class="picker-select" @change=${this._typeChanged}>
          <option value="" ?selected=${!t}>— Choose a card type —</option>
          ${Je.map((e=>R`
            <option value="${e.value}" ?selected=${e.value===t}>${e.label}</option>
          `))}
        </select>
        ${a?R`<p class="picker-desc">${a.description}</p>`:""}
      </div>
