...
`},Te="soccer_live_cache_",Le=new Map;class Ne{static set(e,t){try{const i=JSON.stringify(t);if(Le.get(e)===i)return;Le.size>=50&&Le.delete(Le.keys().next().value),Le.set(e,i),localStorage.setItem(Te+e,JSON.stringify({timestamp:Date.now(),data:t}))}catch(e){console.warn("Failed to cache:",e)}}static get(e){try{const t=localStorage.getItem(Te+e);if(!t)return null;const{timestamp:i,data:a}=JSON.parse(t),n=Date.now()-i;return n<864e5?{data:a,age:n,isCached:!0}:(this.clear(e),null)}catch(e){return console.warn("Failed to read cache:",e),null}}static clear(e){try{localStorage.removeItem(Te+e)}catch(e){console.warn("Failed to clear cache:",e)}}static clearAll(){try{Object.keys(localStorage).forEach((e=>{e.startsWith(Te)&&localStorage.removeItem(e)}))}catch(e){console.warn("Failed to clear all cache:",e)}}static getAge(e){const t=localStorage.getItem(Te+e);if(!t)return null;try{const{timestamp:e}=JSON.parse(t),i=Date.now()-e,a=Math.floor(i/6e4);if(a<60)return`${a}m ago`;const n=Math.floor(a/60);return n<24?`${n}h ago`:`${Math.floor(n/24)}d ago`}catch(e){return null}}}customElements.define("soccer-live-team",class extends oe{static get properties(){return{hass:{},_config:{},_isLoading:{type:Boolean},showPopup:{type:Boolean},activeMatch:{type:Object},_eventSubscriptions:{type:Array},_toastMessage:{type:String},_toastVisible:{type:Boolean},_toastVariant:{type:String},_weatherBadge:{type:Object},_cachedData:{type:Object}}}setConfig(e){if(!e.entity)throw new Error("Entity required");this._config=e,ue(this,e);const t=["big","huge"].includes(e.score_size)?e.score_size:"normal";this.setAttribute("data-score",t),this._isLoading=!0,this._loadingStarted=Date.now(),this._lastWeatherVenue=null,this.showPopup=!1,this.activeMatch=null,this.showEventToasts=!0===e.show_event_toasts,this.myTeam=(e.my_team||"").toLowerCase(),this.showPreviousMatches=!0===e.show_previous_matches,this.showFormTrend=!0===e.show_form_trend,this._toastMessage="",this._toastVisible=!1,this._toastVariant="goal",this._toastTimer=null}_t(e,t){return pe(e,de(this.hass,this._config),t)}_translatePhase(e){return e?{"regular-season":this._t("phase.regular_season"),"regular season":this._t("phase.regular_season"),"group-stage":this._t("phase.group_stage"),"group stage":this._t("phase.group_stage"),playoffs:this._t("phase.playoffs")}[String(e).toLowerCase()]||e:""}_shouldShowPhase(e){return!!e&&"regular-season"!==String(e).toLowerCase()}connectedCallback(){super.connectedCallback(),this._subscribeToEvents(),this._countdownInterval=setInterval((()=>this.requestUpdate()),3e4),this._loadingTimer=setTimeout((()=>this.requestUpdate()),1e4)}disconnectedCallback(){super.disconnectedCallback(),clearInterval(this._countdownInterval),this._countdownInterval=null,clearTimeout(this._loadingTimer),this._eventSubscriptions&&Array.isArray(this._eventSubscriptions)&&(this._eventSubscriptions.forEach((e=>{"function"==typeof e&&e()})),this._eventSubscriptions=[])}_subscribeToEvents(){this.hass&&this.hass.connection&&(this._eventSubscriptions=[],["soccer_live_goal","soccer_live_yellow_card","soccer_live_red_card"].forEach((e=>{this.hass.connection.subscribeEvents(this._handleSoccerLiveEvent.bind(this),e).then((e=>{"function"==typeof e&&this._eventSubscriptions.push(e)}))})))}_eventBelongsToThisCard(e){if(!this.hass||!this._config)return!1;const t=this.hass.states[this._config.entity];if(!t)return!1;const i=t.attributes.matches||[];if(0===i.length)return!1;const a=i[0];return a.home_team===e.home_team&&a.away_team===e.away_team}_handleSoccerLiveEvent(e){const t=e.event_type,i=e.data;if(this._eventBelongsToThisCard(i)&&this.showEventToasts)if("soccer_live_goal"===t){const e=i.team===i.home_team?"home":"away";requestAnimationFrame((()=>this._triggerGoalCelebration(e,i)))}else this._showEventToast(t,i)}_showEventToast(e,t){let i="",a="goal";"soccer_live_goal"===e?(i=`<strong>${this._t("event.goal").toUpperCase()}!</strong> ${t.player} · ${t.home_team} ${t.home_score} - ${t.away_score} ${t.away_team}`,a="goal"):"soccer_live_yellow_card"===e?(i=`🟨 <strong>${this._t("event.yellow_card")}</strong> · ${t.player}${t.minute?` (${t.minute}')`:""}`,a="yellow"):"soccer_live_red_card"===e&&(i=`🟥 <strong>${this._t("event.red_card")}</strong> · ${t.player}${t.minute?` (${t.minute}')`:""}`,a="red"),i&&(this._toastMessage=i,this._toastVariant=a,this._toastVisible=!0,this._toastTimer&&clearTimeout(this._toastTimer),this._toastTimer=setTimeout((()=>{this._toastVisible=!1,this.requestUpdate()}),4e3),this.requestUpdate())}_triggerGoalCelebration(e,t){const i=this.shadowRoot&&this.shadowRoot.querySelector("ha-card");if(!i)return;i.querySelectorAll(".confetti, .goal-banner, .goal-flash-overlay").forEach((e=>e.remove())),i.classList.remove("goal-flash"),i.offsetWidth,i.classList.add("goal-flash"),setTimeout((()=>i.classList.remove("goal-flash")),1700);const a=document.createElement("div");a.className="goal-flash-overlay",i.appendChild(a),setTimeout((()=>a.remove()),1e3);const n=document.createElement("div");n.className="goal-banner",n.innerHTML='<div class="goal-banner-text">GOAL!</div>',i.appendChild(n),setTimeout((()=>n.remove()),1700);const s=i.querySelector(".score-numbers");s&&(s.classList.remove("goal-scored"),s.offsetWidth,s.classList.add("goal-scored"),setTimeout((()=>s.classList.remove("goal-scored")),1300));const o=i.querySelectorAll(".team-side .team-logo-big"),r="away"===e?o[1]:o[0];r&&(r.classList.remove("scorer-bounce"),r.offsetWidth,r.classList.add("scorer-bounce"),setTimeout((()=>r.classList.remove("scorer-bounce")),1300)),navigator.vibrate&&navigator.vibrate([180,80,180,80,280]),setTimeout((()=>this._showEventToast("soccer_live_goal",t)),600);const l=["#ec4899","#6366f1","#06b6d4","#fbbf24","#10b981","#ef4444"],c=["⚽","🎉","✨","🔥","⭐"];for(let e=0;e<36;e++){const e=document.createElement("div");e.className="confetti",Math.random()>.55?(e.textContent=c[Math.floor(Math.random()*c.length)],e.style.fontSize=14+10*Math.random()+"px",e.style.background="transparent"):(e.style.background=l[Math.floor(Math.random()*l.length)],e.style.borderRadius=Math.random()>.5?"50%":"2px");const t=380*(Math.random()-.5)+"px",a=240*Math.random()+100+"px";e.style.setProperty("--dx",t),e.style.setProperty("--dy",a),e.style.animationDelay=.3*Math.random()+"s",i.appendChild(e),setTimeout((()=>e.remove()),2e3)}}_parseMatchDate(e){if(!e||"N/A"===e)return null;try{const[t,i]=e.split(" "),[a,n,s]=t.split(/[-\/]/).map(Number),[o,r]=(i||"00:00").split(":").map(Number);return new Date(s,n-1,a,o,r)}catch(e){return null}}_liveCountdown(e){if(!e||"pre"!==e.state)return null;const t=this._parseMatchDate(e.date);if(!t)return null;const i=t-new Date;if(i<=0||i>1728e5)return null;const a=Math.floor(i/6e4);if(a<1)return this._t("time.now");if(a<60)return this._t("time.in_n_min",{n:a});const n=Math.floor(a/60);return this._t("time.in_n_h",{n})}getCardSize(){return 4}static getConfigElement(){return document.createElement("soccer-live-team-editor")}static getStubConfig(){return{entity:"sensor.soccerlive",show_event_toasts:!1}}showDetails(e){this.activeMatch=e,this.showPopup=!0}closePopup(){this.showPopup=!1}separateEvents(e){const t=[],i=[],a=[];return e.forEach((e=>{const n=String(e||"");n.includes("Goal")||n.includes("Penalty - Scored")?t.push(this.formatMatchEvent(n)):n.includes("Yellow Card")?i.push(this.formatMatchEvent(n)):n.includes("Red Card")&&a.push(this.formatMatchEvent(n))})),{goals:t,yellowCards:i,redCards:a}}formatMatchEvent(e){const t=e=>this._t(e);let i=String(e||"").trim();i=i.replace(/^Goal\s*-\s*/i,"").replace(/^Yellow Card\s*-\s*/i,"").replace(/^Red Card\s*-\s*/i,"").replace(/^Penalty - Scored\s*-\s*/i,`${t("event.penalty")} - `).replace(/^Header\s*-\s*/i,`${t("event.header")} - `).replace(/^Shot\s*-\s*/i,`${t("event.shot")} - `).replace(/^Free-kick\s*-\s*/i,`${t("event.free_kick")} - `).replace(/^Penalty\s*-\s*/i,`${t("event.penalty")} - `),i=i.replace(/^([^:]+):\s*/,"$1 ");const a=[t("event.header"),t("event.shot"),t("event.penalty"),t("event.free_kick")].map((e=>e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));return i=i.replace(new RegExp(`^(${a.join("|")})\\s*-\\s*(.+)$`,"i"),((e,t,i)=>`${i} (${t.toLowerCase()})`)),i=i.replace(/\bN\/A\b/g,t("generic.unknown")),i}_renderStatusBadge(e){const t=e.state;return"in"===t?R`<span class="status-badge live"><span class="dot"></span>${this._t("status.live")}</span>`:"post"===t?R`<span class="status-badge finished">${this._t("status.finished")}</span>`:R`<span class="status-badge scheduled">${e.date||this._t("status.scheduled")}</span>`}_renderClock(e){const t=e.state;if("in"===t){const t=e.status_detail&&"N/A"!==e.status_detail?e.status_detail:"",i=(e.clock&&"N/A"!==e.clock?e.clock:"")||t||e.status||"";return R`<div class="clock"><span class="dot"></span>${i}</div>`}if("post"===t)return R`<div class="clock finished">${this._t("status.full_time")}</div>`;const i=this._liveCountdown(e);return R`<div class="clock upcoming">${i||e.date||""}</div>`}_renderRecord(e){if(!e||"N/A"===e)return"";const t=String(e).split("-");return 3===t.length?t.every((e=>0===parseInt(e)))?"":R`<div class="record">
    ...
    `}}),window.customCards=window.customCards||[],window.customCards.push({type:"soccer-live-live-commentary",name:"Soccer Live Commentary",description:"Live match commentary and events"});const Ue=["dark","light","red-white","classic","neon","gold"],Ge=["auto","en","nl","de","pt","fr","es","it"];customElements.define("soccer-live-live-commentary-editor",class extends oe{static get properties(){return{_config:{type:Object},hass:{type:Object},entities:{type:Array}}}constructor(){super(),this.entities=[]}static get styles(){return[ve,s`
      .hint { font-size: 11px; color: var(--secondary-text-color); }
    `]}setConfig(e){this._config={...e}}updated(e){e.has("hass")&&this._fetchEntities()}_fetchEntities(){this.hass&&(this.entities=Object.keys(this.hass.states).filter((e=>e.includes("soccerlive_commentary")||e.includes("soccer_live_commentary"))).sort())}_fire(e){this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0})),this.requestUpdate()}_entityChanged(e){this._fire({...this._config,entity:e.target.value})}_selectChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.value})}_switchChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.checked})}render(){if(!this._config||!this.hass)return R``;const e=this._config.entity||"",t=e&&this.entities.includes(e);return R`
      <div class="card-config">
        <h3>Sensor</h3>
