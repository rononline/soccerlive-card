...
`;async function we(e){if(!e||"N/A"===e)return q``;try{const t=await async function(e){if(!e||"N/A"===e)return null;if(ve.has(e))return ve.get(e);if(xe[e])return ve.set(e,xe[e]),xe[e];try{const t=await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(e+" stadium")}&format=json&limit=1`,{headers:{"Accept-Language":"en"}}),a=await t.json();if(a&&a.length>0){const t={lat:parseFloat(a[0].lat),lon:parseFloat(a[0].lon)};return ve.set(e,t),t}}catch(e){console.warn("Geocoding failed:",e)}return null}(e);if(!t)return q``;const a=await async function(e,t){const a=`${e},${t}`;if(be.has(a)){const e=be.get(a);if(Date.now()-e.timestamp<36e5)return e.data}try{const s=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${e}&longitude=${t}&current=temperature_2m,weather_code,wind_speed_10m&timezone=auto`),n=await s.json();if(n.current){const e={temp:Math.round(n.current.temperature_2m),code:n.current.weather_code,wind:Math.round(n.current.wind_speed_10m),icon:(i=n.current.weather_code,i&&0!==i&&1!==i?2===i?"⛅":3===i?"☁️":45===i||48===i?"🌫️":51===i||53===i||55===i||61===i||63===i||65===i?"🌧️":71===i||73===i||75===i||77===i?"🌨️":80===i||81===i||82===i?"🌧️":85===i||86===i?"🌨️":95===i||96===i||99===i?"⛈️":"🌤️":"☀️"),description:_e(n.current.weather_code),timestamp:Date.now()};return be.set(a,{data:e,timestamp:Date.now()}),e}}catch(e){console.warn("Weather fetch failed:",e)}var i;return null}(t.lat,t.lon);return a?q`
      ...
      <ha-card>
        <div class="hero">
          ...
          <div class="teams-row">
            ...
            <div class="score-center">
              ${a?q`<div class="live-badge">LIVE</div>`:i?q`<div class="sched-info">FT</div>`:q`<div class="sched-info">${t.date||"vs"}</div>`}
              ${a||i?q`<div class="score">${t.home_score??0} - ${t.away_score??0}</div>`:q`<div class="vs-text">vs</div>`}
    ...
    `]}_renderMatch(e){const t=this.hass.states[e];if(!t)return q`<div class="match-row"><div class="no-match">Unknown: ${e}</div></div>`;const a=this._getMatch(t);if(!a)return q`<div class="match-row"><div class="no-match">No data for ${e}</div></div>`;const i="in"===a.state,s="post"===a.state,n=i||s,o=a.date||"";return q`
      <div class="match-row">
        ...
        <div class="score-block">
          ...
          ${n?q`<div class="score">${a.home_score??0} - ${a.away_score??0}</div>`:q`<div class="status">${o||"vs"}</div>`}
    ...
    `}render(){if(!this.hass||!this._config)return q``;const e=this._config.entities||[];return e.length?q`
      <ha-card>
        ${this._config.hide_header?"":q`<div class="title">${this._config.title||"My Teams"}</div>`}
