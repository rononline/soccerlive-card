...
`;async function $e(e){let t=arguments.length>1&&void 0!==arguments[1]?arguments[1]:null,a=arguments.length>2&&void 0!==arguments[2]?arguments[2]:null;if(!e||"N/A"===e)return U``;try{const i=await async function(e){if(!e||"N/A"===e)return null;if(be.has(e))return be.get(e);if(xe[e])return be.set(e,xe[e]),xe[e];try{const t=await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(e+" stadium")}&format=json&limit=1`,{headers:{"Accept-Language":"en"}}),a=await t.json();if(a&&a.length>0){const t={lat:parseFloat(a[0].lat),lon:parseFloat(a[0].lon)};return be.set(e,t),t}}catch(e){console.warn("Geocoding failed:",e)}return null}(e);if(!i)return U``;const s=await async function(e,t){const a=`${e},${t}`;if(_e.has(a)){const e=_e.get(a);if(Date.now()-e.timestamp<36e5)return e.data}try{const n=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${e}&longitude=${t}&current=temperature_2m,weather_code,wind_speed_10m&timezone=auto`),o=await n.json();if(o.current){const e=o.current.wind_speed_10m,t={temp:Math.round(o.current.temperature_2m),code:o.current.weather_code,wind:(s=e,s<1?0:s<6?1:s<12?2:s<20?3:s<29?4:s<39?5:s<50?6:s<62?7:s<75?8:s<89?9:s<103?10:s<118?11:12),wind_unit:"BFT",icon:(i=o.current.weather_code,i&&0!==i&&1!==i?2===i?"⛅":3===i?"☁️":45===i||48===i?"🌫️":51===i||53===i||55===i||61===i||63===i||65===i?"🌧️":71===i||73===i||75===i||77===i?"🌨️":80===i||81===i||82===i?"🌧️":85===i||86===i?"🌨️":95===i||96===i||99===i?"⛈️":"🌤️":"☀️"),description:ye(o.current.weather_code),timestamp:Date.now()};return _e.set(a,{data:t,timestamp:Date.now()}),t}}catch(e){console.warn("Weather fetch failed:",e)}var i,s;return null}(i.lat,i.lon);if(!s)return U``;const n=t?pe("weather.wind",de(t,a)):"Wind speed (Beaufort)";return U`
    ...
    `]}setConfig(e){this._config={...e}}updated(e){e.has("hass")&&this._fetchEntities()}_fetchEntities(){this.hass&&(this.entities=Object.keys(this.hass.states).filter((e=>e.includes("soccerlive_next"))).sort())}_fire(e){this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0})),this.requestUpdate()}_entityChanged(e){this._fire({...this._config,entity:e.target.value})}_selectChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.value})}_switchChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.checked})}render(){if(!this._config||!this.hass)return U``;const e=this._config.entity||"",t=e&&this.entities.includes(e);return U`
      <div class="card-config">
        ...
        <div>
          <label class="field-label">Theme</label>
          <select data-config-value="skin" @change=${this._selectChanged}>
            ${je.map((e=>U`<option value="${e}" ?selected=${(this._config.skin||"dark")===e}>${e}</option>`))}
          </select>
        </div>
        <div>
          <label class="field-label">Language</label>
