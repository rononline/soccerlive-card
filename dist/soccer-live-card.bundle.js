...
`;async function we(e){let t=arguments.length>1&&void 0!==arguments[1]?arguments[1]:null,a=arguments.length>2&&void 0!==arguments[2]?arguments[2]:null;if(!e||"N/A"===e)return q``;try{const i=await async function(e){if(!e||"N/A"===e)return null;if(ve.has(e))return ve.get(e);if(_e[e])return ve.set(e,_e[e]),_e[e];try{const t=await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(e+" stadium")}&format=json&limit=1`,{headers:{"Accept-Language":"en"}}),a=await t.json();if(a&&a.length>0){const t={lat:parseFloat(a[0].lat),lon:parseFloat(a[0].lon)};return ve.set(e,t),t}}catch(e){console.warn("Geocoding failed:",e)}return null}(e);if(!i)return q``;const s=await async function(e,t){const a=`${e},${t}`;if(be.has(a)){const e=be.get(a);if(Date.now()-e.timestamp<36e5)return e.data}try{const n=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${e}&longitude=${t}&current=temperature_2m,weather_code,wind_speed_10m&timezone=auto`),o=await n.json();if(o.current){const e=o.current.wind_speed_10m,t={temp:Math.round(o.current.temperature_2m),code:o.current.weather_code,wind:(s=e,s<1?0:s<6?1:s<12?2:s<20?3:s<29?4:s<39?5:s<50?6:s<62?7:s<75?8:s<89?9:s<103?10:s<118?11:12),wind_unit:"BFT",icon:(i=o.current.weather_code,i&&0!==i&&1!==i?2===i?"⛅":3===i?"☁️":45===i||48===i?"🌫️":51===i||53===i||55===i||61===i||63===i||65===i?"🌧️":71===i||73===i||75===i||77===i?"🌨️":80===i||81===i||82===i?"🌧️":85===i||86===i?"🌨️":95===i||96===i||99===i?"⛈️":"🌤️":"☀️"),description:xe(o.current.weather_code),timestamp:Date.now()};return be.set(a,{data:t,timestamp:Date.now()}),t}}catch(e){console.warn("Weather fetch failed:",e)}var i,s;return null}(i.lat,i.lon);if(!s)return q``;const n=t?pe("weather.wind",de(t,a)):"Wind speed (Beaufort)";return q`
    ...
    `}}),customElements.define("soccer-live-live-match",class extends oe{static get properties(){return{hass:{},_config:{}}}setConfig(e){if(!e.entity)throw new Error("Entity required");this._config=e,ue(this,e)}getCardSize(){return 5}_t(e,t){return pe(e,de(this.hass,this._config),t)}static getConfigElement(){return document.createElement("soccer-live-live-match-editor")}static getStubConfig(){return{entity:"sensor.soccerlive_next_"}}_getMatch(e){const t=e.attributes.matches||[];return t.find((e=>"in"===e.state||"live"===e.status))||t.find((e=>"pre"===e.state||"scheduled"===e.status))||t.find((e=>"post"===e.state))||t[0]||null}_eventIcon(e){if(!e)return"📋";const t=(e||"").toLowerCase();return t.includes("goal")||t.includes("score")?"⚽":t.includes("yellow")?"🟨":t.includes("red")?"🟥":t.includes("sub")||t.includes("substitut")?"🔄":t.includes("pen")?"⚽🎯":"📋"}static get styles(){return[he,n`
    ...
    `]}render(){if(!this.hass||!this._config)return q``;const e=this.hass.states[this._config.entity];if(!e)return q`<ha-card><div class="empty">Unknown entity: ${this._config.entity}</div></ha-card>`;const t=this._getMatch(e);if(!t)return q`<ha-card><div class="empty">No match data</div></ha-card>`;const a="in"===t.state||"live"===t.status,i="post"===t.state,s=("pre"===t.state||t.status,t.key_events||t.events||t.match_events||[]),n=t.competition_name||e.attributes.league_name||"",o=t.competition_logo||e.attributes.league_logo||"",r=t.venue&&"N/A"!==t.venue?t.venue:"",l=t.venue_city&&"N/A"!==t.venue_city?t.venue_city:"",c=t.stats||t.statistics||[];return q`
      <ha-card>
        <div class="hero">
    ...
    `}setConfig(e){this._config={...e}}updated(e){e.has("hass")&&this._fetchEntities()}_fetchEntities(){this.hass&&(this.entities=Object.keys(this.hass.states).filter((e=>e.includes("soccerlive_next")||e.includes("soccerlive_all"))).sort())}_fire(e){this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0})),this.requestUpdate()}_entityChanged(e){this._fire({...this._config,entity:e.target.value})}_selectChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.value})}_numberChanged(e){const t=parseInt(e.target.value,10);isNaN(t)||this._fire({...this._config,[e.target.dataset.configValue]:t})}render(){if(!this._config||!this.hass)return q``;const e=this._config.entity||"",t=e&&this.entities.includes(e);return q`
      <div class="card-config">
        ...
        <div>
          <label class="field-label">Theme</label>
          <select data-config-value="skin" @change=${this._selectChanged}>
            ${Ae.map((e=>q`<option value="${e}" ?selected=${(this._config.skin||"dark")===e}>${e}</option>`))}
          </select>
        </div>
        <div>
          <label class="field-label">Language</label>
          <select data-config-value="language" @change=${this._selectChanged}>
            ${Te.map((e=>q`<option value="${"auto"===e?"":e}" ?selected=${(this._config.language||"")===("auto"===e?"":e)}>${e}</option>`))}
          </select>
        </div>
      </div>
    ...
    `}setConfig(e){this._config={...e,entities:[...e.entities||[]]}}updated(e){e.has("hass")&&this._fetchEntities()}_fetchEntities(){this.hass&&(this._allEntities=Object.keys(this.hass.states).filter((e=>e.includes("soccerlive_next")||e.includes("all_mixed"))).sort())}_fire(e){this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0})),this.requestUpdate()}_selectChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.value})}_switchChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.checked})}_textChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.value})}_entityChanged(e,t){const a=[...this._config.entities||[]];a[t]=e.target.value,this._fire({...this._config,entities:a})}_removeEntity(e){const t=[...this._config.entities||[]];t.splice(e,1),this._fire({...this._config,entities:t})}_addEntity(){const e=[...this._config.entities||[],""];this._fire({...this._config,entities:e})}render(){if(!this._config||!this.hass)return q``;const e=this._config.entities||[];return q`
      <div class="card-config">
        ...
        <div>
          <label class="field-label">Theme</label>
          <select data-config-value="skin" @change=${this._selectChanged}>
            ${je.map((e=>q`<option value="${e}" ?selected=${(this._config.skin||"dark")===e}>${e}</option>`))}
          </select>
        </div>
        <div>
          <label class="field-label">Language</label>
          <select data-config-value="language" @change=${this._selectChanged}>
            ${Ne.map((e=>q`<option value="${"auto"===e?"":e}" ?selected=${(this._config.language||"")===("auto"===e?"":e)}>${e}</option>`))}
          </select>
        </div>
      </div>
    ...
    `}setConfig(e){this._config={...e}}updated(e){e.has("hass")&&this._fetchEntities()}_fetchEntities(){this.hass&&(this.entities=Object.keys(this.hass.states).filter((e=>e.includes("all_mixed"))).sort())}_fire(e){this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0})),this.requestUpdate()}_entityChanged(e){this._fire({...this._config,entity:e.target.value})}_selectChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.value})}_numberChanged(e){const t=parseInt(e.target.value,10);isNaN(t)||this._fire({...this._config,[e.target.dataset.configValue]:t})}_textChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.value})}_switchChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.checked})}render(){if(!this._config||!this.hass)return q``;const e=this._config.entity||"",t=e&&this.entities.includes(e);return q`
      <div class="card-config">
        ...
        <div>
          <label class="field-label">Theme</label>
          <select data-config-value="skin" @change=${this._selectChanged}>
            ${Me.map((e=>q`<option value="${e}" ?selected=${(this._config.skin||"dark")===e}>${e}</option>`))}
          </select>
        </div>
        <div>
          <label class="field-label">Language</label>
          <select data-config-value="language" @change=${this._selectChanged}>
            ${Le.map((e=>q`<option value="${"auto"===e?"":e}" ?selected=${(this._config.language||"")===("auto"===e?"":e)}>${e}</option>`))}
          </select>
        </div>
      </div>
    ...
    `}setConfig(e){this._config={...e}}updated(e){e.has("hass")&&this._fetchEntities()}_fetchEntities(){this.hass&&(this.entities=Object.keys(this.hass.states).filter((e=>e.includes("soccerlive_commentary"))).sort())}_fire(e){this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0})),this.requestUpdate()}_entityChanged(e){this._fire({...this._config,entity:e.target.value})}_selectChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.value})}_switchChanged(e){this._fire({...this._config,[e.target.dataset.configValue]:e.target.checked})}render(){if(!this._config||!this.hass)return q``;const e=this._config.entity||"",t=e&&this.entities.includes(e);return q`
      <div class="card-config">
        ...
        <div>
          <label class="field-label">Theme</label>
          <select data-config-value="skin" @change=${this._selectChanged}>
            ${Pe.map((e=>q`<option value="${e}" ?selected=${(this._config.skin||"dark")===e}>${e}</option>`))}
          </select>
        </div>
        <div>
          <label class="field-label">Language</label>
          <select data-config-value="language" @change=${this._selectChanged}>
            ${De.map((e=>q`<option value="${"auto"===e?"":e}" ?selected=${(this._config.language||"")===("auto"===e?"":e)}>${e}</option>`))}
          </select>
        </div>
      </div>
    `}})})();