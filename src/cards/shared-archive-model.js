...
export function archiveResult(match, teamName) {
  ...
  return { code: own > other ? 'w' : own < other ? 'l' : 'd', own, other };
}

export function archiveModel(matches, teamName, season = '', competition = '') {
  ...
  return { matches: filtered, seasons, competitions, stats };
}
